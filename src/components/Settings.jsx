...
export default function Settings() {
    ...
    async function handleTelegramSave() {
        ...
        if (!telegramChatId.trim()) {
            setTelegramStatus({
                type: "error",
                message: "Telegram Chat ID cannot be empty."
            });
            return;
        }
    ...
    async function handleIGConnect() {
        ...
        const missing = Object.entries(igAccount)
            .filter(([_, v]) => !v)
            .map(([k]) => k);
        ...
        if (missing.length) {
            setIgStatus({
                type: "error",
                message: "All IG account fields are required."
            });
            return;
        }
        ...
        setIgStatus({
            type: "success",
            message: "IG account data captured. Check console."
        });
    }

    const card = "bg-[#11161d] border border-[#1f2933] rounded-xl p-5";
    const input =
        "w-[80%] sm:w-[70%] lg:w-[60%] p-2 rounded bg-[#0b0f14] border border-[#1f2933] text-gray-100 block";
    const btn =
        "px-4 py-2 text-sm font-semibold rounded border border-[#1f2933] transition";
  const statusBox = (s) =>
    s && (
        <div
            className={`mt-3 p-3 rounded border text-sm transition-opacity duration-500 ${s.type === "success"
    ...
    return (
        <div className="min-h-screen bg-[#0b0f14] text-gray-100 p-6">
            <div className="mx-auto max-w-[60%] space-y-6">
                ...
                <div className={`${card} flex items-center gap-4`}>
                    {user.photoURL ? (
                        <img
                            src={user.photoURL}
                            alt="Avatar"
                            className="w-16 h-16 rounded-full border border-[#1f2933]"
                        />
                    ) : (
                ...
                {isPasswordUser && (
                    <div className={card}>
                        ...
                        <button
                            ...
                            className={`${btn} mt-3 text-green-400 hover:text-green-300`}
                        >
                            Save Password
                        </button>
                ...
                <div className={card}>
                    ...
                    <button
                        ...
                        className={`${btn} mt-3 text-blue-400 hover:text-blue-300`}
                    >
                        Save Chat ID
                    </button>
                ...
                <div className={card}>
                    ...
                    {["username", "password", "accountID", "accountType"].map(f => (
                        <input
                            ...
                            value={igAccount[f]}
                            onChange={e =>
                                setIgAccount({ ...igAccount, [f]: e.target.value })
                            }
                        />
                    ))}
                    ...
                    <button
                        ...
                        className={`${btn} text-purple-400 hover:text-purple-300`}
                    >
                        Connect IG Account
                    </button>
